  ...
  <section class="panel ridge">
    ...
    <div class="ridge-body">
      <div class="ridge-stats">
        <span><TermHint term="有效数据源" /> <b>{{ observation?.length ? '2 / 2' : '0 / 2' }}</b></span>
        <span><TermHint term="已扫描" /> <b>{{ observation?.length ? '2' : '0' }}</b></span>
        <span><TermHint term="已准入" /> <b>{{ observation?.length ? '查看报告' : '—' }}</b></span>
      </div>
      <div class="ridge-visual">
        ...
        <div class="ridge-label">
          扣除手续费后净收益<br />
          <b>{{ observation?.length ? '已计算' : '等待数据' }}</b>
        </div>
      </div>
    </div>
  </section>
