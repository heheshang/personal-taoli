...
| R-3 `Dirty` 需人工处置 | 半应用后应用无法自动继续 | 需人工检查后修复。**运维动作未文档化到 runbook**，记录为待补。**2026-09-10 首次实测（G-02 的 0008 迁移失败）**：失败走的是**回滚**而非 `Dirty`——sqlx 把每个迁移文件与其记账放在同一事务，故 `_sqlx_migrations` 无失败行、schema 无残留，修正文件后直接重跑即成功。风险等级相应下调，但仍保留：真正的半应用只可能来自 `-- no-transaction` 文件 |
