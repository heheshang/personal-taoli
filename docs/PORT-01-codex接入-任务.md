...
| PORT-01-R | 实测：指令写法无提升（已回退，零差异） | Q | **`verified`**（见下，结论为否） |
| PORT-01-S | 修复结构化输出失败根因（跨轮次指令冲突）+ 显示原因 | Q/R | **`verified`**（见下） |
...
| PORT-01-W | 未按结构定义时原样展示模型返回的结构 | V | **`verified`**（见下） |
| PORT-01-X | chat 内渲染 HTML 报告（沙箱容器，含对抗性验证） | W | **`verified`**（见下） |
...
## PORT-01-R 实测：把要求写进领域指令**没有**提升遵循率（已回退）（2026-09-10）

**背景**：上一轮（Q）我把「结构化报告不稳定」列为局限，并提出两条改进方向，其中第 ① 条是
「把要求写进 `docs/agent/instructions.md` 作为长期领域指令」。所有者答「需要」，故做了并**实测**。

**假设**：领域指令里加一节「结构化输出」，会比每轮的 `additionalContext` 更能提高遵循率。

**方法**（受控对照）：两臂**只**在「指令里是否有这一节」上不同——同一 schema、同一
`additionalContext` 片段、同一提示词、同一线程设置。每臂 3 轮。判定标准：
`TurnOutcome.report.is_some()`，它本身就意味着最终消息**解析为 JSON 且通过了 schema 的三个必需字段校验**；
产出散文或自创字段名都记为失败（这正是实测观察到的两种失败模式）。
...
1. **基线已在天花板。** 3/3 无从改善，n=3 的实验**检测不出提升**——这是实验的局限，
   不能读成「证明该节有害」，但同样**没有任何证据支持保留它**。
2. **唯一那次失败落在「加了规则」那一侧，而且很可能是我自己造成的。** 失败原因是
   `最终回复不是 JSON`（即输出了散文）。而我写的规则里有一条：

   > 若你判断无法产出合格的结构化结果，请在最终回复里**直接说明原因**……

   **我给了模型一个逃生口，它用了。** 这条规则不仅没帮上忙，还引入了它本该防止的失败模式。
3. **该节内容基本是重复的。** 既有规则已经覆盖了它想说的两件事：
   - `## 4. 不确定：显式标注，不猜测` 已写明「不得**编造字段名**、时间、数量或错误信息」；
   - `## 2. 结论必须写在回复里` 已要求最终消息就是交付物。

**决定：回退。** `docs/agent/instructions.md` 已回到与改动前**字节一致**（`git diff` 无输出），
本节不保留。净效果是本轮**代码与文档零差异**——这一轮的价值是知识，不是改动。

**对设计的意义（未变）**：schema 仍是**建议性**的，校验仍是**必需**的，`additionalContext`
片段仍保留（更早的实测：只发 schema 而没有任何说明时，模型**直接拒绝**，说「未提供给定结构」——
即片段是把它从「拒绝」推到「产出 JSON」的那一步）。因此界面**必须**继续假设报告可能缺席并优雅回退。

**若要真正提高遵循率，需要的是机制而非措辞**，例如：把 schema 收窄到更少字段（字段越少越容易满足），
或用多次采样取首个通过校验的结果。两者都未做，也都不在本轮范围内。

**实验可复现**：探针为一次性脚本（已按惯例删除），方法如上——两臂仅差指令文本、各 3 轮、
以「产出通过校验的结构化报告」为判定。复现需要一个可用的 codex 与约 15 分钟。

---

## PORT-01-S 修掉「未按结构输出」的根因：我埋下的指令冲突（2026-09-10）

**现场表现**（所有者实际运行）：分析完成后，chat 界面显示「未按结构输出，已回退为正文」。

**为何一开始无法诊断**：我**存了** `report_error` 原因，却**没有显示它**。界面只给结论、
不给依据，于是「模型没按结构」与「模型写错了字段」这两种完全不同的失败无法区分——而它们要
做的事相反（前者改指示，后者记录模型能力）。**这是我实现上的缺口，先补上**：原因现在既可见
也在 `title` 中。

**根因（从原因文本定位）**：原因原文是
`最终回复不是 JSON（expected value at line 1 column 1）`。
`line 1 column 1` 意味着回复**第一个字符就不是 JSON**——极可能是被 ``` 围栏包住或被说明文字
包裹。而这正是**我跨轮次埋下的指令冲突**：

| 来源 | 它说的是 |
|---|---|
| 领域指令 §2（轮次 O 加，为散文输出而写） | 正文**按 Markdown 组织**，「代码块」明确可用 |
| 本轮附加上下文（轮次 Q 加，为结构化输出而写） | 回复必须是**纯 JSON**，不要任何额外文字 |

两条直接打架。而 §2 是 **developer instruction**（更强的一档），片段只是**附加上下文**；
引用「更权威的指令」时，模型跟了 §2。**这不是模型不听话，是我的指示自相矛盾。**

**修复（三处）**：
1. **领域指令 §2 增例外条款**：本轮若要求了结构化输出，则最终那条回复就是该结构本身；本节所说
   的正文由结构里的文字字段（如 `summary`）承载，Markdown 只在该字段内部使用；**不要**用围栏包裹
   整条回复；过程性汇报不受此限。这不是新增要求，是**消解与既有条款的冲突**。
2. **上下文片段改为直接点名冲突**：明说「整条回复就是这一个 JSON，不要用 ``` 包裹」，
   并解释「本轮『正文必须出现在回复里』由 `summary` 承载」——把两条指示的**关系**讲清楚，
   而不是只重复一边。
3. **界面显示失败原因**（见上）。

**实测（同一集成测试，真实运行时 + 真实 schema + 真实指令）**

| 阶段 | 通过 |
|---|---|
| 轮次 R 对照：不含结构化节 | 3/3 |
| 轮次 R 对照：含结构化节（含「说明原因」逃生口） | 2/3 |
| **本次修复后** | **6/6** |

**不要过度解读**：n=6 的 95% 置信下界约 0.60，这个样本量**不足以证明比例提升**。可断定的是
**机制**——此前存在一条真实冲突（两条指令互斥，且更强的那条与目标相反），现已消解；而修复后
的 6 次全部通过，与「冲突是主因」一致。

**顺带说明轮次 R 的失败为何同一根因**：那一轮「加了规则」反而 2/3，我当时归因于自己写的
「无法产出就说明原因」逃生口。本次证据显示更可能的主因是**这条跨轮次冲突**——R 的两臂都带着
§2 + 片段的矛盾，模型在两种编排之间摇摆；而 R 只统计 3 次，把摇摆读成了「规则有害」。两次测量
的错误归因都被记录在此，避免重复。

**验收**：`cargo fmt/clippy(-D warnings)/test` clean / 0 / **323 core + 38 tauri**；
`npm run build` 零错误；渲染断言确认原因可见（chip + 详情 + title），零横向溢出、零页面错误。

**仍未验证**：真实 Tauri 宿主（沿用 mock IPC）；更大的遵循率样本（要真正量化需几十次运行，
成本约数小时——若将来值得，用 `TurnOutcome.report` 的通过率做批测即可）。

---

## PORT-01-T 步骤图标（按命令族）+ 修掉自动滚动抢占用户滚动（2026-09-10）

### T1 步骤图标

**背景**：所有者指令——「chat 中不同的命令使用不同的图标进行展示优化」。

**实现**（`src/components/StepIcon.vue`）：两级识别，因为 chat 记录要一眼回答两个问题：
1. **这是什么步骤**——取自运行时的 item 类型（reasoning / command / tool_call / file_change / …），
   不是猜测。
2. **具体是哪类命令**——命令文本上的**启发式**，且**仅影响呈现**：认不出来就回落到通用终端图标，
   失误的代价是图标不够具体，绝不会影响正确性。

**一个必须处理的细节**：运行时把命令**包在 shell 里**执行，所以文本是
`/bin/zsh -lc 'git status'`。若直接按首词判断，看到的永远是 `/bin/zsh`——即**每个命令都是通用
终端图标**。故先剥外壳（`unwrapShell`），再剥透明前缀（`sudo`/`env`/…）与环境变量赋值，最后取
首词并与命令族表比对。命令族刻意保持简短（git/python/node/cargo/grep/cat/curl 等），
命名集中在一处，便于看清它**覆盖什么、不覆盖什么**。

**顺带的两处呈现改动**：原先「步骤类型」是一个文字 chip，现在由图标承担类型、由**颜色**承担
状态（运行中橙并呼吸、成功绿、失败红），这一行因此不必再挤一个标签；展开指示从文字 `▶` 换成
真正的 SVG chevron。

**一次由实测纠正的判断**：初版 `read`（读取文件）用「文档+横线」，与 `file_change` 的
「文档+加号」**共用同一个文档轮廓**，16px 下几乎分不出来。改为**眼睛**——轮廓不同比细节不同更
重要，小而常看的东西靠剪影区分。

### T2 修掉自动滚动抢占用户滚动（所有者报告的缺陷）

**现场表现**：分析进行中，chat 界面**无法上滑**。

**复现**（构造一个每次轮询都增长的模拟会话）：
```
手动滚到顶部 -> scrollTop = 0
3 秒后（几次轮询）-> scrollTop = 202   ← 被自动拽回底部
```

**根因**（精确，非猜测）：自动滚动的 watch 写成了
```js
watch(() => [turns.length, status.phase, pending.request_id], () => scrollToBottom())
```
getter **返回数组字面量**，每次求值都是新引用；Vue 以 `Object.is` 比较返回值，因此**永远判定为
「变了」**，这个 watch 在**每次轮询**（1 秒一次）都触发。本意是「只在变化时触发」，而那个写法
根本没有「不变」这回事——自动滚动成了每秒一次、无法挣脱的拽动。

**修复两处**：
1. **getter 改为标量**（各项长度之和）。既表达了「内容变长了」，也真的只在长度变化时触发。
2. **自动跟随必须让位于用户**：新增 `pinnedToBottom`，由会话流的 `scroll` 事件维护；滚离底部即
   **停手**，滚回底部即恢复。`force` 只用于「用户的动作本身表示要看结果」的两处——发出提问、
   打开/切回本页。
...
| 初始 | 202 / 202 | 打开即到底部 ✓ |
| 用户上滑后 | 0 | 用户滚离 |
| **3.5 秒轮询后** | **0** | **停在用户离开的位置** ✓（修复前为 202） |
| 滚回底部后 | 202 | 恢复跟随 ✓ |

**其他验收**：10 行步骤 → **10 个互异图标**（以完整 markup 判定，非截断）；chevron 展开时旋转
`codex-caret` → `codex-caret open`；失败的工具调用行着红（`codex-step-icon bad`）；运行中的行有
呼吸动画且尊重 `prefers-reduced-motion`；零横向溢出、零页面错误。

**门禁**：`cargo fmt/clippy(-D warnings)/test` clean / 0 / **323 core + 38 tauri**；
`npm run build` 零错误。

**未验证**：真实 Tauri 宿主（沿用 mock IPC）；命令族表的覆盖度只在常见命令上核过，未穷举。

---

## PORT-01-U 修掉「工具调用后看不到结果」：步骤在轮次结束时被清空（2026-09-10）

**现场表现**（所有者报告）：chat 里工具调用之后，无法在对应的工具下面看到结果。

**复现**（按后端当时真实的表示构造一个已完成轮次：`live = null`、`tool_calls = []`）：
```
stepRows: 0        ← 一行步骤都没有
proseBlocks: 1     ← 只剩模型的文字
threadText: 分析 002600.SZ | 已完成分析。
```

**根因（读码 + 复现双证）**：两条记录渠道都留不下运行时执行的命令。

| 渠道 | 实际装了什么 |
|---|---|
| `turn.tool_calls` | **只有** `item/tool/call`（宿主工具）的分发。运行时自己执行的命令从不进入它。而当前宿主**不注册任何工具**，故它恒为空 |
| `live.items` | 命令、工具、文件变更、推理**全都在这里**——但轮次结束时 `guard.live = None` **把它清掉了** |

两者叠加的结果：步骤与输出在轮次进行中可见，**一结束就整体消失**。对话里只剩下模型的散文。
我先前在清空处写的注释是「记录已具权威性，保留实时副本会重复渲染同一批工作」——那个前提**当时
就不成立**，因为记录里根本没有这些步骤。

**修复**：轮次结束时把实时步骤**移入**记录（`take()`，不复制输出字符串），而不是丢弃。
- `AgentTurn.items: Vec<AgentLiveItem>`（与实时视图同型，故渲染可共用）。
- 清空实时视图由这次 `take()` 顺带完成，不再需要显式置 `None`。

**顺带消除的重复**：步骤行的 markup 此前在三处各写一遍（实时、宿主工具调用、以及现在需要的
留存步骤），已抽成 `StepRow.vue` 供前两者共用。重复是我先前那次缺陷的土壤——本轮把记录渲染
写成第三份，而其中两份本来就该是同一份。
...
| 项 | 结果 |
|---|---|
| 完成一轮后步骤仍在 | ✅ 4 行（修复前 **0** 行） |
| 输出挂在对应步骤下面 | ✅ 展开第 3 行 → 正文为 `stage1 done\nrecords=284\n`，`aria-expanded="true"` |
| 默认折叠，不倾泻输出 | ✅ 展开前 `tool-body` 计数为 0 |
| 空输出不产生空正文框 | ✅ `emptyBodies: 0` |
| 实时路径回归（共用组件后） | ✅ 2 行、状态为「完成」与「进行中… · partial output」、运行中行有呼吸；展开显示增量输出 |
| 零横向溢出 / 零页面错误 | ✅ |
...
## PORT-01-W 未按结构定义时，原样展示模型返回的结构（2026-09-10）

**背景**：所有者指令——「最终回复不是 JSON（expected value at line 1 column 1）的情况下展示原来的元数据」，
并在澄清中选定：**保留模型实际返回的结构，以通用元数据原样展示**。

**此前的问题**：后端把「解析成功但不符合 schema」的对象**丢弃**了，只在 `report_error` 里留下原因。
而实测显示，模型忽略 schema 时仍会返回「某种」结构化内容（只是字段名自创，如
`stock_code`/`company_name`，或中文的 `业务结构_2026H1`）——**一次数分钟的调用因为三个键名不同就被丢掉**。

**实现**：
- `report::TurnReport { value, conforms }`：**值 + 是否符合**放在同一个对象里，而不是两个可空字段。
  后者会容许「有标志却没值」这种不可能状态，也会让渲染方去猜。现在的状态只有两种：
  没有报告，或「这份报告，符合或不符合」。
- 不符合也**保留**，并把原因一并给出；只有「压根不是 JSON」时才没有结构可留（那时回复就是散文，
  已在 `messages` 里，界面按正文显示）。
- 新增 `ReportMetadata.vue`：形状**未知**，故只做一种区分——
  **标量**给「字段→值」一行（可读）；**对象/数组**整体放进可折叠的 JSON 块，
  因为替 `业务结构_2026H1` 猜一版版式，等于把猜测当成设计。
- 字段名**原样保留**（包括看起来像笔误的 `stock_code`）：那个差异**正是诊断信息**，
  在此改名或归一化会把要传达的东西藏起来。

**顺带修掉一处措辞已不准确**：原先的 chip 是「未按结构输出，**已回退为正文**」。
在「结构被保留并展示」之后这句话是**错的**——正文并没有取代结构，两者并有。现按两种情况分开：
- 结构存在但不符合 → 面板自带的「不符合结构定义」，**不再**出现该 chip；
- 压根不是 JSON → chip 改为「回复不是 JSON，已按正文显示」+ 具体原因。

**验收（两种情形分别断言）**

| 情形 | 结果 |
|---|---|
| 结构存在但不符合 schema | ✅ `report-raw` 面板；徽章「不符合结构定义」「原样展示」；说明行含原因与实际字段名；标量 `stock_code → 002600.SZ`、`company_name → 领益智造` 原样；结构化字段折叠为 `data 对象`、`业务结构_2026H1 数组 1 项`；**无**矛盾的「回退为正文」chip；散文块 1（JSON 消息未重复） |
| 压根不是 JSON | ✅ 无面板；chip「未按结构输出，回复不是 JSON，已按正文显示」+ 详情 `最终回复不是 JSON（expected value at line 1 column 1）`；散文块 2（原文以代码块可见） |
| 零横向溢出 / 零页面错误 | ✅ 两种情形均 0 |
| 视觉确认 | ✅ 面板整洁：警示底、徽章、说明行三行换行不裁切、字段表两行对齐、无重叠 |

**门禁**：`cargo fmt/clippy(-D warnings)/test` clean / 0 / **323 core + 40 tauri**；
`npm run build` 零错误。

**未验证**：真实 Tauri 宿主（沿用 mock IPC）；返回值极深或极大时的折叠块长度（当前原样输出 JSON，
未加上限）。

---

## PORT-01-X chat 内渲染 agent 生成的 HTML 报告（沙箱容器）（2026-09-10）

**背景**：所有者指令——「chat 界面支持 html 最终结果进行 html 渲染」，并补充「类似以前的分析结果功能，
这次要在 chat 聊天界面支持展示」。即：富版式的报告在**对话内**呈现（`分析结果` 独立页已在 P 轮次移除）。

**先讲清风险，因为它推翻了我自己建的一道防线**：markdown 渲染里 `html: false` 是我上一轮特意设的，
理由写在代码注释里——模型输出不可信，而落点是一个能调用 Rust 后端的 webview。直接渲染原始 HTML 意味着
模型（或其读到的网页内容）写一句 `<img onerror="…invoke(…)">` 就能执行后端命令。故本轮的实现不在
「关掉转义」，而是**给 HTML 一个容器**。

**容器（`HtmlFrame.vue`）**：
- `sandbox=""`：空 token 列表是最严格设置——禁脚本、禁表单、禁顶层跳转、独立不透明源。脚本是决定性的一项：
  一旦禁用，载荷就够不到 `window.__TAURI_INTERNALS__`，敌意文档因此只是**惰性**的，而非能调用后端命令。
- 文档内 `default-src 'none'`：无脚本、无样式表、无字体、**无远程资源**。报告本应自足；拦掉远程获取，
  同时去掉了 iframe 本来留着的追踪像素与数据外泄通路。
- `referrerpolicy="no-referrer"`。
- 两条代价**明说而非等人踩到**：**脚本不执行**，故用 JS 画图的报告会退化为静态标记；**远程图片与样式表
  不加载**，文档必须自带 CSS、内联图片（`data:`）——这正是「自足报告」本来的含义。

**选择哪种渲染器（`isHtmlDocument`）**：末条 agent 消息若**自称文档**才走 frame，其余走 Markdown。
判定刻意保守——必须**以标记开头**（`<!doctype html` / `<html` / `<body`，或以结构元素开头且以 `>` 结尾），
因为两条路径的信任处理完全不同（Markdown 转义 HTML，frame 容纳它），这个选择必须显式。
渲染过的文档仍**可核对**：出处行（「HTML 文档」「已在沙箱中渲染 · 脚本与外部资源被禁用」）+ 源码开关。

**验证（对抗性输入 + 地面真相，而非读策略下结论）**

| 攻击 | 结果 |
|---|---|
| `<script>parent.__PWNED_SCRIPT__=1; window.__TAURI_INTERNALS__.invoke('agent_start')</script>` | ✅ **未执行**（`__PWNED_SCRIPT__ = 0`）；脚本站元素在 DOM 里存在但不运行 |
| `<img src=x onerror="parent.__PWNED_ONERROR__=1">` | ✅ **未执行**（`__PWNED_ONERROR__ = 0`） |
| `<img src="http://127.0.0.1:4191/from-frame.gif">`（指向本地计数服务器） | ✅ **服务器未收到任何请求**——帧内会显示一次失败的 `request`，但网络层没有到达。**这是我用「服务器收没收到」定的论**，不是读 CSP 得出的结论 |
| `sandbox` 属性 | ✅ `""`（最严格） |
| 文档自身 CSS | ✅ 生效（`h1` 计算字号 22px，来自文档的 `<style>`） |

**边界不误判**（四种输入，逐一断言）：

| 输入 | 期望 | 结果 |
|---|---|---|
| `结论见 <table> 那一节；<TICKER> 的涨跌幅是 -1.2%。` | 散文 | ✅ 0 frame / 2 散文 |
| 代码围栏里的 `<div>hi</div>` | 散文 | ✅ 0 frame / 2 散文 |
| `<div><h1>报告</h1><p>正文</p></div>` | 文档 | ✅ 1 frame / 1 散文 |
| `<!doctype html><html>…` | 文档 | ✅ 1 frame / 1 散文 |

**富报告在 chat 内的呈现**（所有者所说的「类似以前的分析结果」）：一份带 KPI 卡片与表格的
自足 HTML 报告在对话内完整渲染——标题与副标题、3 张卡片（`最新价=12.97`、`涨跌幅=-1.22%`、
`PE(TTM)=24.3`）、4 行表格、其自带 CSS 生效；frame 1236×480，页面零横向溢出。

**固定高度而非自动撑高**：沙箱禁用了脚本，因而**无法从文档内部测得内容高度并回报**，「自动撑高」
在这里做不到。给一个够读的高度让文档自己滚动，比猜一个高度诚实。

**门禁**：`cargo fmt/clippy(-D warnings)/test` clean / 0 / **323 core + 40 tauri**；
`npm run build` 零错误。

**未验证**：真实 Tauri 宿主（沿用 mock IPC）—— CSP 与 `sandbox` 均由 WebKit/WKWebView 实现，
行为与 Chromium 可能不同，**上线前应在真实宿主里复核一次**；frame 内的滚动与深色模式未核。
