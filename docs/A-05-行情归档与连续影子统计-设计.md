...
- `src/main.rs`：CLI、实时数据编排、扫描/健康事件选择、关闭信号和归档生命周期。
- `src/archive.rs::DecisionEvent`：冻结一次决策的全部重算输入与保存输出。
- `src/archive.rs::ArchiveWriter`：分配运行 ID/序号、非阻塞提交、单线程追加写、缺口审计和优雅关闭。
- `src/archive.rs::replay_archive`：固定长度顺序流式读取、结构与顺序校验、确定性重算、统计聚合。
- `src/config.rs::ArchiveConfig`：归档路径、队列容量和保留期元数据校验。
- `config/observer.toml`：唯一配置样例；正式窗口通过 `--archive` 覆盖路径，不复制第二套配置。
...
- `--archive PATH` 是对 `config.archive.path` 的运行时覆盖，避免为正式窗口维护第二份配置。
...
| SIGTERM 刷盘 | `shutdown_signal`, `ArchiveWriter::finish` | release 进程受控停止后回放，36 条记录、未完成尾字节 0 |
| 实时在线回放 | CLI `--replay` | 正式窗口首批 15 条记录，30 个方向、回放一致、缺口 0 |
...
`--quiet` 只抑制每轮扫描报告，不抑制启动、跳过、归档错误和关闭信息。进程仍然无订单入口。完整启动、停止、在线核对和最终评审见《A-05 连续影子观察运行与评审说明》。