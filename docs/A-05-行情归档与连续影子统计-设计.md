...
- `crates/observer-cli/src/main.rs`：CLI 参数、信号处理和连续观察进程入口。
...
- `config/observer.toml`：唯一配置样例；正式窗口通过 CLI `--archive` 覆盖路径，不复制第二套配置。
...
```bash
target/release/personal-taoli-observer \
