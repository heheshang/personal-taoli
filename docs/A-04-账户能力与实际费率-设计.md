...
- `src/config.rs`：环境变量名、HTTP/认证窗口、刷新间隔、最大费率年龄与人工资格确认的配置校验。
- `src/account.rs::capability`：静态能力卡和官方来源。
- `src/account.rs::Credentials`：秘密注入和 `Zeroizing` 生命周期。
- `src/account.rs::load_account_data`：两所并发加载、错误隔离和回退。
- `src/account.rs::{load_binance,load_bybit}`：场所权限与费率组合。
- `src/account.rs::{binance_get,bybit_get}`：认证、HTTP 状态和场所错误归一化。
- `src/account.rs::AccountData::admission_rejections`：动态费率有效性门禁。
- `src/main.rs`：启动加载、独立账户检查、持续刷新和安全输出。
- `src/scan.rs`：合并账户拒绝原因并将实际买卖侧费率映射到两个套利方向。
...
- `--account-check`：加载一次后输出 `[AccountData; 2]` 并退出，不启动行情流。
- `--once`：先加载账户数据，再建立公共行情簿并扫描；账户不合格时仍输出带拒绝原因的观察结果。
- 持续模式：公共行情循环不因账户失败退出；账户定时刷新后下一次扫描使用新数组。
- 文本输出仅打印费率来源、买卖费率、时间、只读状态、资格确认和拒绝原因；JSON 序列化 `AccountData`，不包含凭证。

## 11. 验收映射

| 需求 | 场景 | 主要符号 | 验证方式 |
|---|---|---|---|
| A04-R02 | 无凭证回退 | `Credentials::from_env`、`fallback_account` | 本地无凭证 `--account-check`；配置/账户单元边界 |
...
| A04-R01、A04-R08 | 能力与独立账户检查 | `capability`、`print_accounts` | 真实无凭证 `--account-check --output json` |
| A04-R07 | 两所独立加载与失败回退 | `load_account_data`、`load_venue` | 并行编排和错误归一化行为检查 |
| A04-R09 | 秘密不输出 | `AccountData` 序列化边界、`print_account` | 类型、错误脱敏与真实 CLI 输出检查 |
